    ...
    include-in-header:
      - text: |
          ...
          <style>
            ...
            i[class*="fa-"], i[class*="ai-"], i[class*="bi-"]{ margin-right: .35rem; vertical-align: -0.12em; display:inline-block }
            /* Package link icons */
...
::: {.cv-hero}
::: {.cv-hero-copy}
<div class="cv-hero-name">Jeffrey S. Racine</div>

Senator William McMaster Chair in Econometrics · Professor of Economics · Professor, Graduate Program in Statistics

Nonparametric econometrics, mixed-data smoothing, reproducible econometrics, and R package development.
:::

::: {.cv-hero-actions}
[Download PDF](vitae.pdf){.cv-button .cv-button-primary}
[Research](https://jeffreyracine.github.io/research/){.cv-button}
[Code Gallery](https://jeffreyracine.github.io/gallery/){.cv-button}
[GitHub](https://github.com/JeffreyRacine){.cv-button}
[Google Scholar](https://scholar.google.com/citations?user=TXeKxGsAAAAJ){.cv-button}
[ORCID](https://orcid.org/0000-0002-5680-3705){.cv-button}
:::
:::

:::: {.cv-summary-grid}
::: {.cv-summary-card}
<div class="cv-summary-title">Current Positions</div>

McMaster Economics, Graduate Statistics, Senator William McMaster Chair in Econometrics, Info-Metrics Institute, and Rimini Center for Economic Analysis.
:::

::: {.cv-summary-card}
<div class="cv-summary-title">Research Contributions</div>

Three books, edited volumes, peer-reviewed articles, reviews, invited lectures, and workshops in econometrics and statistics.
:::

::: {.cv-summary-card}
<div class="cv-summary-title">R Packages on CRAN</div>

Lead author and maintainer of `np`, `npRmpi`, `crs`, and related R software for nonparametric and semiparametric methods.
:::

::: {.cv-summary-card}
<div class="cv-summary-title">Live Google Scholar Metrics</div>

Google Scholar citation counts and figures are refreshed by a local scheduled render and then served as static GitHub Pages output.
:::
::::

::: {.cv-official-note}
This HTML page is a web companion to the official PDF CV. For circulation, use the [PDF version](vitae.pdf).
:::
:::
...
-   [<i class="fa-solid fa-file-lines" aria-hidden="true"></i> CV Website: jeffreyracine.github.io/vitae](https://jeffreyracine.github.io/vitae/)
-   [<i class="bi bi-code-slash" aria-hidden="true"></i> Gallery of Code: jeffreyracine.github.io/gallery](https://jeffreyracine.github.io/gallery/)
...
-   [<i class="fa-solid fa-book-open-reader" aria-hidden="true"></i> Research Website: jeffreyracine.github.io/research](https://jeffreyracine.github.io/research/)
...
-   Associate Editor, [*Econometric Reviews*](http://www.tandfonline.com/action/journalInformation?show=editorialBoard&journalCode=lecr20), (2012-2025)
...
-   Visiting Fellow, ENSAI (École nationale de la statistique et de l'analyse d l'information), Rennes, France (2026)
...
if(!"All" %in% names(gse_tbl) && ncol(gse_tbl) >= 2) names(gse_tbl)[2] <- "All"
if(!"Since 2021" %in% names(gse_tbl) && ncol(gse_tbl) >= 3) names(gse_tbl)[3] <- "Since 2021"
...
if(nrow(gse_tbl) >= 3) gse_tbl[[1]][1:3] <- c("Citations", "h-index", "i10-index")
...
if(knitr::is_latex_output()) {
  ...
  fmt_num <- function(x) format(as.numeric(x), big.mark = ",", scientific = FALSE, trim = TRUE)
  metric_col <- if("index" %in% names(gse_tbl)) "index" else names(gse_tbl)[1]
  all_col <- if("All" %in% names(gse_tbl)) "All" else names(gse_tbl)[2]
  recent_col <- if("Since 2021" %in% names(gse_tbl)) "Since 2021" else tail(names(gse_tbl), 1)

  metric_cards <- vapply(seq_len(nrow(gse_tbl)), function(i) {
    sprintf(
      '<article class="gs-metric-card"><div class="gs-metric-label">%s</div><div class="gs-metric-values"><span><em>All</em><strong>%s</strong></span><span><em>%s</em><strong>%s</strong></span></div></article>',
      html_escape(gse_tbl[[metric_col]][i]),
      html_escape(fmt_num(gse_tbl[[all_col]][i])),
      html_escape(recent_col),
      html_escape(fmt_num(gse_tbl[[recent_col]][i]))
    )
  }, character(1))

  cat(
    '<div class="gs-native gs-metrics" aria-label="Google Scholar citation metrics">',
    paste(metric_cards, collapse = "\n"),
    '</div>',
    sep = "\n"
  )
}
...
adjust <- gse_tbl$All[1]-sum(foo$overall_citations$n_citations)
if(knitr::is_latex_output()) {
  require(ggplot2)
  require(gridExtra)
  ...
  fmt_num <- function(x) format(round(as.numeric(x)), big.mark = ",", scientific = FALSE, trim = TRUE)
  citation_data <- foo$overall_citations
  citation_data$cumulative_citations <- cumsum(citation_data$n_citations) + adjust

  make_chart <- function(data, value_col, title, subtitle, chart_class) {
    values <- as.numeric(data[[value_col]])
    years <- as.integer(data$year)
    max_value <- max(values, na.rm = TRUE)
    if(!is.finite(max_value) || max_value <= 0) max_value <- 1
    show_year <- years %% 5 == 0 | seq_along(years) == 1 | seq_along(years) == length(years)

    bars <- vapply(seq_along(values), function(i) {
      height <- max(2, 100 * values[i] / max_value)
      year_label <- if(show_year[i]) html_escape(years[i]) else "&nbsp;"
      sprintf(
        '<div class="gs-bar-wrap"><div class="gs-bar" style="height: %.2f%%" title="%s: %s"></div><div class="gs-year">%s</div></div>',
        height,
        html_escape(years[i]),
        html_escape(fmt_num(values[i])),
        year_label
      )
    }, character(1))

    sprintf(
      '<article class="gs-chart %s"><div class="gs-chart-header"><h3>%s</h3><span>%s</span></div><div class="gs-bars" aria-label="%s">%s</div></article>',
      chart_class,
      html_escape(title),
  ...
  cat(
    '<div class="gs-native gs-charts">',
    make_chart(citation_data, "n_citations", "Citations by Year", "Annual Google Scholar citations", "gs-chart-annual"),
    make_chart(citation_data, "cumulative_citations", "Cumulative Citations", "Running total adjusted to the current Scholar total", "gs-chart-cumulative"),
    '</div>',
    sep = "\n"
  )
}
...
1.  University of Miami (Department of Economics), August 21, 2026, "Semiparametric Covariance Estimation for Sparse Functional Data."

1.  The 7th Conference of the International Symposia for Nonparametric Statistics, June 24, 2026, Thessaloniki, Greece, "Semiparametric Covariance Estimation for Sparse Functional Data."

2.  McMaster University (Graduate Program in Statistics), March 12, 2026, "Boundary Adjusted, Polynomial Adaptive, Nonparametric Kernel Conditional Density Estimation."

3.  The University of Washington, Seattle, February 18, 2025, "Boundary Corrected Adaptive Conditional Kernel Density Estimation."

4.  The 6th Conference of the International Symposia for Nonparametric Statistics, June 25, 2024, Braga Portgugal, "Locally Adaptive Online Functional Data Analysis."

4.  XIII Foro Internacional de Estadística Aplicada (keynote speaker), Veracruz, Mexico, May 22, 2024, "Locally Adaptive Online Functional Data Analysis."

5.  2024 Econometrics Workshop, University of Kansas, April 27, 2024, "Locally Adaptive Online Functional Data Analysis."

6.  6TH Workshop on Goodness-Of-Fit, Change-Point and Related Problems, Skukuza Lodge, Kruger National Park, South Africa, August 27, 2023, "Locally Adaptive Online Functional Data Analysis."

7.  University of Pretoria (Department of Economics), August 21, 2023, "Locally Adaptive Online Functional Data Analysis."

8.  Georgia Institute of Technology (Department of Economics), April 28, 2023, "Locally Adaptive Online Functional Data Analysis."

9.  Emory University (Department of Economics), April 24, 2023, "Locally Adaptive Online Functional Data Analysis."

10. 2nd International Congress of Applied Statistics, Escuela Superior Politecnica de Chimborazo, Riobamba, Ecuador (keynote speaker), April 19, 2023, "Locally Adaptive Online Functional Data Analysis."

11. McMaster University (Graduate Program in Statistics), April 4, 2023, "Locally Adaptive Online Functional Data Analysis."

12. McGill University (Department of Economics), March 10, 2023, "Locally Adaptive Online Functional Data Analysis."

13. The 5th Conference of the International Symposia for Nonparametric Statistics, June 23, 2022, Paphos Cyprus, "Boundary-Adaptive Kernel Density Estimation."

14. New Directions in Economic Theory and Empirical Economics Conference (keynote speaker), Indian Institute of Management Kolkata, August 17, 2018, "Dealing with model uncertainty via frequentist model averaging."

15. Indian Statistical Institute Kolkata, August 10, 2018, "Bootstrap Model Averaging Unit Root Inference."

16. Universidad del Rosario, April 19, 2018, "Bootstrap Model Averaging Unit Root Inference."
...
1.  Scientific Committee member, *International Symposia for Nonparametric Statistics*, June 22-26, 2026, held in Thessaloniki, Greece.

2.  Session Chair, "New Directions in Nonparametric Methods," *International Symposia for Nonparametric Statistics*, June 24, 2026, held in Thessaloniki, Greece.

3.  Introduced keynote speaker Xuming He, *International Symposia for Nonparametric Statistics*, June 24, 2026, held in Thessaloniki, Greece.

4.  Session Chair, "Topics in Econometrics I," *International Symposia for Nonparametric Statistics*, June 25, 2026, held in Thessaloniki, Greece.

5.  Session Chair, "Estimation, Semiparametrics and Prediction," *International Symposia for Nonparametric Statistics*, June 25, 2026, held in Thessaloniki, Greece.

6.  Member of Scientific Committe, Conference Volume Guest Editor, Session Organizer and Chair, *International Society for Nonparametric Statistics*, June 25-29, 2024, held in Braga, Portugal.

7.  Discussant for "Kernel Estimation in Regression on Vector and Function Spaces" by Sid Kankanala, Marcia Schafgans, and Victoria Zinde-Walsh, the Canadian Econometrics Study Group, McMaster University, Hamilton Ontario, October 28, 2023.

8.  Session Organizer and Chair, *International Symposia for Nonparametric Statistics*, June 20-24, 2022, held in Paphos, Cyprus.
